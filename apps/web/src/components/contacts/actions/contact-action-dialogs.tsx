...
export function ContactConfirmationDialog({
  ...
  const title = isDelete
    ? `Delete ${contactCount > 1 ? `${contactCount} Contacts` : `Contact "${contactName}"`}?`
    : `Archive ${contactCount > 1 ? `${contactCount} Contacts` : `Contact "${contactName}"`}?`;

  const description = isDelete
    ? "This action is permanent and will remove all contact records, channel history, and document associations."
    : "Archived contacts will be hidden from default views but remain preserved for historical auditing.";
