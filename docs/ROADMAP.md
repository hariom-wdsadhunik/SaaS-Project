...
- **v3.6.0 (Enterprise BI & Reporting Platform):** Centralized Reporting Engine, 16-Metric KPI Library, Executive Dashboards, Custom Drag-and-Drop Dashboard Builder with 10 chart types, Scheduled Email Reports, Multi-Format Exporter, and `/reports` UI sub-app.
- **v3.7.0 (Enterprise AI Intelligence Platform):** Centralized AI Intelligence Engine, Predictive Lead & Opportunity Scoring, Deal Win Probability, Churn Risk, Revenue Forecasting with 85-95% Confidence Intervals, Next Best Action Engine, Explainable AI (XAI) Insights, and `/ai` UI sub-app.
- **v3.8.0 (Enterprise Administration & Operations Platform):** Global Admin Console, Feature Flags & Canary Rollouts, Infrastructure Telemetry Monitoring, Background Job Queue Manager, Security Operations Center (SOC), Backup & Disaster Recovery Console, and `/admin` UI sub-app (`/admin`, `/admin/system`, `/admin/organizations`, `/admin/users`, `/admin/jobs`, `/admin/monitoring`, `/admin/feature-flags`, `/admin/audit`).
