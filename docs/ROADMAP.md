...
- **v3.5.0 (Enterprise Integrations Hub):** Modular Connector Framework supporting Google Workspace, Microsoft 365, Communication Providers (Twilio, WhatsApp, SendGrid, SMTP), Stripe Payments, Webhooks Engine with HMAC security, Scoped API Keys, and `/integrations` UI sub-app (`/integrations`, `/integrations/google`, `/integrations/microsoft`, `/integrations/communication`, `/integrations/payments`, `/integrations/webhooks`, `/integrations/api-keys`).
