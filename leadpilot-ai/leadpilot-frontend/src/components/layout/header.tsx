    ...
    <header className="sticky top-0 z-20 flex h-14 w-full items-center justify-between border-b border-zinc-800 bg-zinc-950/80 px-4 backdrop-blur-md">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="relative">
          ...
          >
            <Avatar src={user?.avatarUrl} fallback={user?.fullName?.[0] || "A"} size="sm" />
            <ChevronDown className="h-3.5 w-3.5 text-zinc-400" />
          </button>

          {isUserMenuOpen && (
            <div className="absolute right-0 mt-2 w-60 rounded-xl border border-zinc-800 bg-zinc-900 p-1.5 shadow-2xl z-50 animate-in fade-in duration-100">
              <div className="px-3 py-2 border-b border-zinc-800/80 space-y-1">
                <p className="text-xs font-semibold text-zinc-200">{user?.fullName || "Alex Morgan"}</p>
                <p className="text-[11px] text-zinc-500 truncate">{user?.email || "alex@leadpilot.ai"}</p>
                <div className="flex items-center gap-1.5 pt-1">
                  <span className="inline-flex items-center gap-1 rounded bg-indigo-500/10 px-2 py-0.5 text-[10px] font-bold text-indigo-400 border border-indigo-500/20 font-mono">
