...
import { UserCheck, Layers, X } from "lucide-react";
import { Button } from "@/components/ui/button";

interface DealStatusAssignModalProps {
  isOpen: boolean;
  mode: "ASSIGN" | "STAGE";
  itemCount: number;
...
export function DealStatusAssignModal({
  isOpen,
  mode,
  itemCount,
  isProcessing = false,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-100 select-none">
      <div
        role="dialog"
        aria-modal="true"
        aria-label={isAssign ? "Assign Broker Agent" : "Update Pipeline Stage"}
      ...
      >
        <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400">
              {isAssign ? <UserCheck className="h-4 w-4" /> : <Layers className="h-4 w-4" />}
            </div>
            <div>
              <h3 className="text-sm font-bold text-white">
                {isAssign
                  ? `Assign Broker (${itemCount} Deals)`
                  : `Change Stage (${itemCount} Deals)`}
              </h3>
              <p className="text-[11px] text-zinc-400">
                {isAssign
                  ? "Select broker agent to take ownership of opportunity"
                  : "Update pipeline stage across selected deal records"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="space-y-2 py-2">
          <label className="text-xs font-medium text-zinc-300">
            {isAssign ? "Select Broker Agent" : "Select Target Stage"}
          </label>
          {isAssign ? (
            <select
              value={activeValue}
